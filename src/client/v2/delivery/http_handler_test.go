...
import (
	...
	clientUCV2Mock "github.com/Bhinneka/user-service/src/client/v2/usecase/mocks"
...
func TestNewHTTPHandler(t *testing.T) {
	type args struct {
		...
		clientUC        cu.ClientUsecase
		clientV2UC      cuV2.ClientUsecase
	}
	tests := []struct {
		name string
		args args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			NewHTTPHandler(tt.args.authUseCase, tt.args.activityService, tt.args.clientUC, tt.args.clientV2UC)
		})
	}
}

func TestHTTPClientHandler_Mount(t *testing.T) {
	type fields struct {
		AuthUseCase     au.AuthUseCase
		ActivityService service.ActivityServices
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{},
			args: args{
				group: echo.New().Group(testing.CoverMode()),
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := &HTTPClientHandler{
				...
				ClientUsecase:   tt.fields.ClientUsecase,
				ClientV2Usecase: tt.fields.ClientV2Usecase,
			}
			h.Mount(tt.args.group, tt.args.mf)
		})
	}
}
...
func TestHTTPClientHandler_Login(t *testing.T) {
	for _, tc := range testDataLogin {
		...
		mockClientV2Usecase := new(clientUCV2Mock.ClientUsecase)
		...
		handler := NewHTTPHandler(mockAuthUseCase, mockActivityService, mockClientUsecase, mockClientV2Usecase)
		err := handler.Login(c)
...
func TestHTTPClientHandler_Logout(t *testing.T) {
	for _, tc := range testDataLogout {
		...
		mockClientV2Usecase.On(logout, mock.Anything, mock.Anything).Return(createSharedResult(tc.responseLogout))
		handler := NewHTTPHandler(mockAuthUseCase, mockActivityService, mockClientUsecase, mockClientV2Usecase)
...
func TestHTTPClientHandler_VerifyToken(t *testing.T) {
