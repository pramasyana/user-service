...
func TestGetMembers(t *testing.T) {
	tests := []basicInput{
		...
		},
		{
			...
			token:           tokenAdmin,
			wantUsecaseData: usecase.ResultUseCase{Result: defaultResult},
			wantStatusCode:  http.StatusInternalServerError,
			wantError:       true,
		},
	}
...
func TestFindDetailProfile(t *testing.T) {
	tests := []struct {
		name            string
		token           string
		wantUsecaseData usecase.ResultUseCase
...
func TestUpdatePassword(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockMemberUsecase.On("SendEmailSuccessForgotPassword", mock.Anything, mock.Anything).Return(generateUsecaseResult(tt.wantUsecaseData2))
...
func TestAddPassword(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:             testCaseNegative4,
			token:            tokenAdmin,
			wantUsecaseData1: usecase.ResultUseCase{Result: defaultResult},
			wantStatusCode:   http.StatusInternalServerError,
			wantError:        true,
		},
		{
...
func TestRegisterMember(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:             testCaseNegative5,
			token:            tokenAdmin,
			wantUsecaseData2: usecase.ResultUseCase{Result: defaultResult},
			wantStatusCode:   http.StatusInternalServerError,
			wantError:        true,
		},
...
func TestActivationMember(t *testing.T) {
	tests := []struct {
		name                              string
		token                             string
		wantUsecaseData, wantUsecaseData2 usecase.ResultUseCase
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockMemberUsecase.On("SendEmailWelcomeMember", mock.Anything, mock.Anything).Return(generateUsecaseResult(tt.wantUsecaseData2))
...
func TestChangeForgotPassword(t *testing.T) {
	tests := []struct {
		name                              string
		token                             string
		wantUsecaseData, wantUsecaseData2 usecase.ResultUseCase
...
func TestActivateNewPassword(t *testing.T) {
	tests := []struct {
		name            string
		token           string
		wantUsecaseData usecase.ResultUseCase
...
var (
	errDefault   = fmt.Errorf("myemail@domain.com")
	errDefNoDesc = fmt.Errorf("some error")
	defMember    = []*model.Member{
		{
			ID: "somestring",
		},
	}
...
func TestImportMember(t *testing.T) {
	tests := []struct {
		...
		token                           string
		args                            string
		resultInvalidRows, resultImport usecase.ResultUseCase
		wantError                       bool
		wantStatusCode                  int
		parseMemberData                 []*model.Member
		parseError                      error
	}{
		{
			name:            testCasePositive1,
			token:           tokenAdmin,
			resultImport:    usecase.ResultUseCase{Result: model.SuccessResponse{}},
			wantStatusCode:  http.StatusCreated,
			args:            fileBase64,
			parseMemberData: defMember,
		},
		{
			name:              testCaseNegative2,
			token:             tokenAdmin,
			resultInvalidRows: usecase.ResultUseCase{Error: errDefault},
			wantStatusCode:    http.StatusBadRequest,
			args:              fileBase64,
			wantError:         true,
			parseError:        errDefNoDesc,
		},
		{
			name:              testCaseNegative3,
			token:             tokenAdmin,
		...
		},
		{
			...
			token:           tokenAdmin,
			wantStatusCode:  http.StatusBadRequest,
			resultImport:    usecase.ResultUseCase{Error: errDefault},
			args:            fileBase64,
			parseMemberData: defMember,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockMemberUsecase := new(mocksMember.MemberUseCase)
			mockMemberUsecase.On("ParseMemberData", mock.Anything, mock.Anything).Return(tt.parseMemberData, tt.parseError)
			mockMemberUsecase.On("CheckEmailAndMobileExistence", mock.Anything, mock.Anything).Return(generateUsecaseResult(tt.resultInvalidRows))
			mockMemberUsecase.On("ImportMember", mock.Anything, mock.Anything).Return(generateUsecaseResult(tt.resultImport))
...
func TestGetDetailMember(t *testing.T) {
	tests := []struct {
		name            string
		token           string
		wantUsecaseData usecase.ResultUseCase
