...
var (
	defaultTotalPhoneAres = model.TotalPhoneArea{TotalData: int(10)}
...
func TestHTTPDelivery(t *testing.T) {
	var testData = []struct {
		...
		responseUsecase2 usecase.ResultUseCase
	}{
		{
			name:             "Test #1",
			expectStatusCode: http.StatusOK,
			responseUsecase:  usecase.ResultUseCase{Result: defaultResult},
			responseUsecase2: usecase.ResultUseCase{Result: defaultTotalPhoneAres},
		},
		{
			name:             "Test #2",
			expectStatusCode: http.StatusInternalServerError,
			responseUsecase:  usecase.ResultUseCase{Error: errDefault, HTTPStatus: http.StatusInternalServerError},
		},
		{
			name:             "Test #3",
			expectStatusCode: http.StatusBadRequest,
			responseUsecase:  usecase.ResultUseCase{Result: "some result"},
		},
		{
			name:             "Test #4",
			expectStatusCode: http.StatusBadRequest,
			responseUsecase:  usecase.ResultUseCase{Result: defaultResult},
			responseUsecase2: usecase.ResultUseCase{Error: errDefault, HTTPStatus: http.StatusBadRequest},
		},
		{
			name:             "Test #4",
			expectStatusCode: http.StatusBadRequest,
			responseUsecase:  usecase.ResultUseCase{Result: defaultResult},
			responseUsecase2: usecase.ResultUseCase{Result: "some other result"},
		},
	}
	for _, tc := range testData {
		...
		mockUC.On("GetTotalPhoneArea", mock.Anything).Return(generateUsecaseResult(tc.responseUsecase2))
		e := echo.New()
		...
		handler.MountPhoneArea(e.Group("/v2/phone-area"))
